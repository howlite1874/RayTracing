...
Homogeneous4 RaytraceRenderWidget::getColor(Ray r,int depth,float curIOR){
    ...
    if (ci.tri.isValid()) {
        ...
        if (renderParameters->interpolationRendering == 1) {
            ...
            return n1 * bari.x + n2 * bari.y + n3 * bari.z;
        }
        //blinn phong shading
        float EPS = std::numeric_limits<float>::epsilon();

        if (renderParameters->phongEnabled == 1) {
            ...
            if (renderParameters->monteCarloEnabled && (1.f - tri.shared_material->reflectivity - tri.shared_material->transparency) > EPS && cosine < 0)
            {
                Homogeneous4 indirectLigthing;
                    // enable monteCarlo
                    Ray difuzz_ray = r.getRandomReflect(intersect, N);
                    difuzz_ray.ray_type = r.ray_type;
                    float ambient = (1.f - tri.shared_material->transparency - tri.shared_material->reflectivity);

                    // Simulate indirect lighting using Monte Carlo
                    uint32_t numSamples = 32; // Adjust this number as needed
                    for (uint32_t n = 0; n < numSamples; ++n) {
                        // Generate a new random ray direction
                        Ray sampleRay = difuzz_ray.getRandomReflect(intersect, N);

                        // Recursively trace the sample ray and accumulate the color
                        Homogeneous4 sampleColor = getColor(sampleRay, curIOR, depth - 1);

                        // Compute the cos(theta) factor for the sample ray
                        float cos_theta = sampleRay.direction.dot(N);

                        // Weight the sample color by the cosine factor and the PDF
                        sampleColor = sampleColor * cos_theta / pdf;

                        // Add the weighted sample color to the indirect lighting
                        indirectLigthing = indirectLigthing + sampleColor;
                    }

                    // Normalize the accumulated indirect lighting by the number of samples
                    indirectLigthing = indirectLigthing / numSamples;

                    // Combine the indirect lighting with other terms as needed
                    Homogeneous4 temp_color = ambient * indirectLigthing.modulate(tri.shared_material->ambient * 0.5);
            ...
            if(renderParameters->reflectionEnabled && tri.shared_material->reflectivity > EPS && cosine < 0){
                ...
                reflectedColor=tri.shared_material->reflectivity * getColor(reflectRay,--depth,curIOR);
            }

            //refract
            if(renderParameters->refractionEnabled&&tri.shared_material->transparency>EPS)
            {
                Ray refractRay=refract(r,N,intersect,curIOR/tri.shared_material->indexOfRefraction);
                refractedColor = tri.shared_material->transparency * getColor(refractRay,--depth,tri.shared_material->indexOfRefraction);
            }

            float kr = tri.shared_material->transparency;
            Homogeneous4 color1;
            Homogeneous4 color2;
            if(renderParameters->fresnelRendering && (tri.shared_material->transparency > EPS || !renderParameters->monteCarloEnabled)){
                fresnel(r, N,curIOR,tri.shared_material->indexOfRefraction,kr);
                color1=(1 - tri.shared_material->reflectivity) * color + kr * reflectedColor;
                color2=(1 - tri.shared_material->transparency) * color + (1-kr) * refractedColor;
                color = color1 + color2;
            }
            else{
                color1 = (1 - tri.shared_material->reflectivity) * color +reflectedColor;
                color2 = (1 - tri.shared_material->transparency) * color + refractedColor;
                color = color1 + color2;
            }
        ...
        for (int i = 0; i < len; i++) {
            ...
            if(renderParameters->fresnelRendering){
                float FresnelValue;
                fresnel(r, N,curIOR,tri.shared_material->indexOfRefraction,FresnelValue);
